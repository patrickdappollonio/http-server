...
func (s *Server) PrintStartup() {
	...
	if s.IsTLSEnabled() {
		...
		switch s.activeTLSMode { //nolint:exhaustive // TLSModeOff is handled by the outer if
		case TLSModeBYO:
			fmt.Fprintln(s.LogOutput, startupPrefix, "TLS certificate:", s.TLSCert)
			fmt.Fprintln(s.LogOutput, startupPrefix, "TLS key:", s.TLSKey)
		case TLSModeAuto:
			fmt.Fprintln(s.LogOutput, startupPrefix, "Certificate provider: Let's Encrypt (automatic)")
			if s.TLSEmail != "" {
				fmt.Fprintln(s.LogOutput, startupPrefix, "ACME email:", s.TLSEmail)
			}
		default:
		}
		...
		// Print cert expiry info (available after cert is loaded/provisioned)
